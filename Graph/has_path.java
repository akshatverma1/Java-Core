...
public class has_path {
    ...
    public static boolean has_path(ArrayList<Edges> graph[],int s , int d,boolean visited[]){
        if(s==d){
            return true;
        }
        visited[s] = true;
        for(int i = 0; i<graph[s].size();i++){
            Edges e = graph[s].get(i);
            if(!visited[e.Destition] && has_path(graph, e.Destition, d, visited)) return true;
        }
        return false;
    }
    public static void main(String arr[]){
        ...
        Creating(graph);
        System.out.print(has_path(graph,0,9, new boolean[V]));
